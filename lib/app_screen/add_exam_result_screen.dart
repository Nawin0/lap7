...
class AddExamResultScreen extends StatefulWidget {
  ...
  const AddExamResultScreen({super.key, this.exam_result ,this.courses,});
          ...
          IconButton(
            onPressed: () async {
              ...
              if (rt == 200) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                      content: Text('Failed to add exam_result. Error code: $rt')),
                );
              }
            },
...
Future<int> addExamResult(ExamResult exam_result) async {
  final response = await http.post(
    Uri.parse('http://192.168.1.4/api/exam_result.php'),
    headers: <String, String>{
      'Content-Type': 'application/json; charset=UTF-8',
    },
   body: jsonEncode(<String, dynamic>{
      'exam_result_code': exam_result.studentCode,
      'exam_result_name': exam_result.courseCode,
